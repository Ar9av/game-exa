...
Six complete Phaser 3 games generated end-to-end by the **game-creation-agent** engine using real GPT Image 2 sprites and backgrounds. Each ships with full source, sprite sheets, level data, and `game-state.json` — run `npm install && npm run dev` to play immediately.
...
Assets were generated by the engine using `scripts/gen_game.mjs` — the GDD and level design are authored inline, and only GPT Image 2 (via `FAL_KEY`) is called for sprites and backgrounds:
